...
func _start_battle(enemy_entity: E_Enemy, enemy_visual: EntityVisual) -> void:
	...
	if _party_combatants.is_empty():
		if active_era == C_TimelineEra.Era.FATHER:
			_party_combatants.append(Combatant.from_dict(BattleData.FATHER_STATS))
		else:
			_party_combatants.append(Combatant.from_dict(BattleData.SON_STATS))
			_party_combatants.append(Combatant.from_dict(BattleData.ALLY1_STATS))
			_party_combatants.append(Combatant.from_dict(BattleData.ALLY2_STATS))
