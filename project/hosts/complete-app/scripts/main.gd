...
# Progression — persists across battles so XP accumulates
var _progression_manager: ProgressionManager = null
var _party_combatants: Array[Combatant] = []
# Monster-scaling curve loaded once at startup. Applied in roll_zone_encounter
# per the zone's difficulty_tier so higher-tier zones produce tougher enemies.
var _monster_scaling_curve: LevelCurve = null
...
func _ready() -> void:
	...
	# Load XP + stat curves from content bundles (if available)
	_load_progression_curves()
...
func _start_battle(enemy_entity: E_Enemy, enemy_visual: EntityVisual) -> void:
	...
	# Reuse persistent party if it exists and matches the current era;
	# otherwise build fresh. This keeps XP/level state across battles.
	if _party_combatants.is_empty():
		_party_combatants = _build_party(active_era)
	# Heal party to full between battles (DQ1: inns heal, but for now auto-heal)
	for member in _party_combatants:
		member.hp = member.max_hp
		member.mp = member.max_mp
		member.status_effects.clear()
		member.is_defending = false
	var party_combatants := _party_combatants

	# Prefer zone encounter table rolling when available: the overworld entity
	# is then just a spawn trigger, and the actual monster identity + count +
	# scaling come from the zone's EncounterTable. Falls back to the entity's
	# enemy_type lookup when the zone has no table (towns, unknown zones) or
	# the roll produces no eligible entries for this era.
	var era_string := era_to_string(active_era)
	var zone_id := LocationManager.get_current_zone()
	var enemy_combatants := roll_zone_encounter(zone_id, era_string)
	if enemy_combatants.is_empty():
		...
		enemy_combatants = build_enemy_group(enemy_def)
...
func _on_battle_ended(result: Dictionary) -> void:
	...
	if result.get("won", false) or result.get("fled", false):
		...
		if result.get("won", false):
			...
			ActivityLog.log_msg("Victory! Gained %d EXP, %d gold." % [xp_gain, result.get("gold", 0)])
			# Award XP to all party members
			if _progression_manager:
				for member in _party_combatants:
					_progression_manager.award_xp(member, xp_gain)
		else:
...
func _switch_active_era() -> void:
	# Clear persistent party so it rebuilds for the new era's roster
	_party_combatants.clear()
...
func _build_party(era) -> Array[Combatant]:
	var party: Array[Combatant] = []
	if era == C_TimelineEra.Era.FATHER:
		var stat_curve = _progression_manager.stat_curves.get("Father") as LevelCurve
		if stat_curve:
			var stats := BattleData.build_hero_from_curve("Father", 4, stat_curve, ["hurt"])
			party.append(Combatant.from_dict(stats))
		else:
			party.append(Combatant.from_dict(BattleData.FATHER_STATS))
	else:
		var son_curve = _progression_manager.stat_curves.get("Son") as LevelCurve
		if son_curve:
			var son_stats := BattleData.build_hero_from_curve("Son", 7, son_curve,
				["heal", "healmore", "hurt", "sleep", "stopspell"])
			party.append(Combatant.from_dict(son_stats))
		else:
			party.append(Combatant.from_dict(BattleData.SON_STATS))
		# Allies stay hardcoded until they become vault character entities
		party.append(Combatant.from_dict(BattleData.ALLY1_STATS))
		party.append(Combatant.from_dict(BattleData.ALLY2_STATS))
	return party


func _load_progression_curves() -> void:
	# Try to load curves from the curves-core bundle. These are .tres files
	# generated by the adapter from vault curve pages.
	for slug: String in ["father", "son"]:
		var xp_id := "%s_xp_to_level_curve" % slug
		var stat_id := "%s_stat_growth_curve" % slug
		var xp_res = ContentManager.get_resource("curves-core", xp_id)
		var stat_res = ContentManager.get_resource("curves-core", stat_id)
		var name_key: String = slug.capitalize()  # "Father" / "Son"
		if xp_res is LevelCurve:
			_progression_manager.xp_curves[name_key] = xp_res
		if stat_res is LevelCurve:
			_progression_manager.stat_curves[name_key] = stat_res
	# Monster scaling curve — not per-era, applied to encounter enemies by tier.
	var scale_res = ContentManager.get_resource("curves-core", "monster_scaling_curve")
	if scale_res is LevelCurve:
		_monster_scaling_curve = scale_res
...
# Roll an encounter from a zone's EncounterTable. Returns an
# Array[Combatant] built from the rolled bestiary entry, or an empty
# array if the zone has no table or the roll finds nothing for this era.
#
# Applies the zone's difficulty_tier through the monster_scaling curve
# (loaded once by _load_progression_curves). Empty zone_id short-circuits
# and returns [], so safe zones (towns) never produce a roll.
#
# Static variant remains for tests that stub the lookup table.
func roll_zone_encounter(zone_id: String, era_string: String) -> Array[Combatant]:
	if zone_id.is_empty():
		return []
	...
	if enemy_def == null:
		...
		return []
	return build_enemy_group(enemy_def, tbl.difficulty_tier, _monster_scaling_curve)


# Static version for deterministic unit tests. Takes an explicit table
# rather than reaching into ContentManager.
static func roll_zone_encounter_from_table(tbl: EncounterTable, era_string: String, scaling: LevelCurve = null) -> Array[Combatant]:
	if tbl == null:
		return []
	var bestiary_id := tbl.roll(era_string)
	if bestiary_id.is_empty():
	...
	if enemy_def == null:
		return []
	return build_enemy_group(enemy_def, tbl.difficulty_tier, scaling)
